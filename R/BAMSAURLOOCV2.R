...
#' @export BAMSAUR.LOOCV

BAMSAUR.LOOCV <- function(object, data = NULL, interval = "prediction", level = 0.68) {
  if(class(object) == "lm"){
...
#lm objects
  #LOOCV
  est <- rstandard(object, interval = interval, level = level)
     est <- age.data - est
...
#earth objects
    ...
    age <- data[,1]
    wear <- data[,2]
    cv.data <- predict(object, wear, type = "earth", interval = "pint", level = level)
    age.diff <- cv.data[,1] - age
    cv.out <- cbind(age, cv.data[,1], age.diff, cv.data[,2], cv.data[,3])
    colnames(cv.out) <- c("age", "est", "diff", "low", "upp")
    cv.out <- as.data.frame(cv.out)
  }
...
#Quantifying relative accuracy, i.e. if the predicted age is within 1, 2, 5, and 10 years of the known age
  ...
  list("out" = cv.out, "accuracy" = round(accuracy, digits = 2), "accuracy.1" = round(accuracy.1, digits = 2), "accuracy.2" = round(accuracy.2, digits = 2), "accuracy.5" = round(accuracy.5, digits = 2), "accuracy.10" = round(accuracy.10, digits = 2))
